...
if __name__ == "__main__":
    ...
    selected_type = "science"
    selected_row_index = 1

    if selected_type == "science":
        df_scores = pd.read_sql("SELECT * FROM science_scores", conn)
        df_passing_grade = pd.read_sql("SELECT * FROM passing_grade WHERE type='science'", conn)
    else:
        df_scores = pd.read_sql("SELECT * FROM humanities_scores", conn)
        df_passing_grade = pd.read_sql("SELECT * FROM passing_grade WHERE type='humanities'", conn)

    conn.close()

    selected_user_id = int(df_scores.iloc[selected_row_index]['id_user'])
    cbf = CBFSystemNormalizedProximity(df_scores, df_passing_grade)
    recommendations = cbf.recommend(user_id=selected_user_id, top_n_user=5, top_n_prodi=10)

    recommendations.to_csv(f"recommendations_{selected_type}.csv", index=False)
